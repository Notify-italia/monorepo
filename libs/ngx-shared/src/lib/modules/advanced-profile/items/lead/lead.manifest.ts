import {
  EnumNotifyAdvancedProfileItems,
  INoitfyAPLeadItem,
} from '@notify/interfaces';
import {
  AdvancedProfileItemsService,
  INotifyAdvancedProfileManifest,
} from '../../services/advanced-profile-items.service';
import { itemManifest } from '../item.manifest';

import { LeadFormComponent } from './lead.form.component';
import { LeadPlayerComponent } from './lead.player.component';

const manifest: INotifyAdvancedProfileManifest<INoitfyAPLeadItem> = {
  type: EnumNotifyAdvancedProfileItems.Lead,
  localizedName: 'Richiesta dati di contatto',
  filledIcon: [
    'M21.731 2.269a2.625 2.625 0 0 0-3.712 0l-1.157 1.157 3.712 3.712 1.157-1.157a2.625 2.625 0 0 0 0-3.712ZM19.513 8.199l-3.712-3.712-8.4 8.4a5.25 5.25 0 0 0-1.32 2.214l-.8 2.685a.75.75 0 0 0 .933.933l2.685-.8a5.25 5.25 0 0 0 2.214-1.32l8.4-8.4Z',
    'M5.25 5.25a3 3 0 0 0-3 3v10.5a3 3 0 0 0 3 3h10.5a3 3 0 0 0 3-3V13.5a.75.75 0 0 0-1.5 0v5.25a1.5 1.5 0 0 1-1.5 1.5H5.25a1.5 1.5 0 0 1-1.5-1.5V8.25a1.5 1.5 0 0 1 1.5-1.5h5.25a.75.75 0 0 0 0-1.5H5.25Z',
  ],
  outlineIcon: [
    'm16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10',
  ],
  formOptions: {
    // hideTextSettings: true,
    // hideTitle: true,
  },
  playerComponent: LeadPlayerComponent,
  formComponent: LeadFormComponent,
  definitions: {
    ...itemManifest.definitions,
    buttonLabel: 'Lasciami il tuo contatto',
    disabledFields: [],
  },
};

AdvancedProfileItemsService.publishManifest(manifest);

import {
  EnumNotifyAdvancedProfileItems,
  INotifyAPDocumentItem,
} from '@notify/interfaces';
import {
  AdvancedProfileItemsService,
  INotifyAdvancedProfileManifest,
} from '../../services/advanced-profile-items.service';
import { itemManifest } from '../item.manifest';
import { DocumentFormComponent } from './document.form.component';
import { DocumentPlayerComponent } from './document.player.component';

const manifest: INotifyAdvancedProfileManifest<INotifyAPDocumentItem> = {
  type: EnumNotifyAdvancedProfileItems.Document,
  localizedName: 'Documento PDF',
  filledIcon: [
    'M3.375 3C2.339 3 1.5 3.84 1.5 4.875v.75c0 1.036.84 1.875 1.875 1.875h17.25c1.035 0 1.875-.84 1.875-1.875v-.75C22.5 3.839 21.66 3 20.625 3H3.375Z',
    'm3.087 9 .54 9.176A3 3 0 0 0 6.62 21h10.757a3 3 0 0 0 2.995-2.824L20.913 9H3.087Zm6.163 3.75A.75.75 0 0 1 10 12h4a.75.75 0 0 1 0 1.5h-4a.75.75 0 0 1-.75-.75Z',
  ],
  outlineIcon: [
    'm20.25 7.5-.625 10.632a2.25 2.25 0 0 1-2.247 2.118H6.622a2.25 2.25 0 0 1-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125Z',
  ],
  formOptions: {
    hideTextSettings: false,
  },
  formComponent: DocumentFormComponent,
  playerComponent: DocumentPlayerComponent,
  definitions: {
    ...itemManifest.definitions,
    docSrc: '',
    showInline: true,
    boxHeight: 200,
    button: {
      icon: '',
      url: '',
      caption: 'Il mio documento',
      visible: true,
    },
  },
};

AdvancedProfileItemsService.publishManifest(manifest);

import { CommonModule } from '@angular/common';
import { Component } from '@angular/core';
import { INotifyLicense } from '@notify/interfaces';
import {
  AuthService,
  CompanyService,
  UtilsService,
} from '@notify/nfc-app-services';
import {
  LicenseFormFactory,
  LicenseInfoComponent,
  LoadingComponent,
  NoItemsComponent,
  PageHeaderComponent,
} from '@notify/ngx-components';
import { catchError, switchMap, tap } from 'rxjs';

@Component({
  standalone: true,
  imports: [
    CommonModule,
    LicenseInfoComponent,
    PageHeaderComponent,
    NoItemsComponent,
    LoadingComponent,
  ],
  providers: [LicenseFormFactory, CompanyService, UtilsService],
  templateUrl: './license.component.html',
  styleUrls: ['./license.component.scss'],
})
export class LicenseComponent {
  public license = this._authService.user?.license as unknown as INotifyLicense;

  public activeLicense = this._authService.activeLicense;
  public daysLeft = 0;

  public loading = false;

  public get headerButtons() {
    const editLicense = {
      label: 'Modifica licenza',
      eventName: 'addLicense',
      buttonColor: 'primary',
      icon: [
        'M11.25 5.337c0-.355-.186-.676-.401-.959a1.647 1.647 0 0 1-.349-1.003c0-1.036 1.007-1.875 2.25-1.875S15 2.34 15 3.375c0 .369-.128.713-.349 1.003-.215.283-.401.604-.401.959 0 .332.278.598.61.578 1.91-.114 3.79-.342 5.632-.676a.75.75 0 0 1 .878.645 49.17 49.17 0 0 1 .376 5.452.657.657 0 0 1-.66.664c-.354 0-.675-.186-.958-.401a1.647 1.647 0 0 0-1.003-.349c-1.035 0-1.875 1.007-1.875 2.25s.84 2.25 1.875 2.25c.369 0 .713-.128 1.003-.349.283-.215.604-.401.959-.401.31 0 .557.262.534.571a48.774 48.774 0 0 1-.595 4.845.75.75 0 0 1-.61.61c-1.82.317-3.673.533-5.555.642a.58.58 0 0 1-.611-.581c0-.355.186-.676.401-.959.221-.29.349-.634.349-1.003 0-1.035-1.007-1.875-2.25-1.875s-2.25.84-2.25 1.875c0 .369.128.713.349 1.003.215.283.401.604.401.959a.641.641 0 0 1-.658.643 49.118 49.118 0 0 1-4.708-.36.75.75 0 0 1-.645-.878c.293-1.614.504-3.257.629-4.924A.53.53 0 0 0 5.337 15c-.355 0-.676.186-.959.401-.29.221-.634.349-1.003.349-1.036 0-1.875-1.007-1.875-2.25s.84-2.25 1.875-2.25c.369 0 .713.128 1.003.349.283.215.604.401.959.401a.656.656 0 0 0 .659-.663 47.703 47.703 0 0 0-.31-4.82.75.75 0 0 1 .83-.832c1.343.155 2.703.254 4.077.294a.64.64 0 0 0 .657-.642Z',
      ],
    };

    const removeLicense = {
      label: 'Richiedi Disdetta',
      icon: [
        'M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25Zm-1.72 6.97a.75.75 0 1 0-1.06 1.06L10.94 12l-1.72 1.72a.75.75 0 1 0 1.06 1.06L12 13.06l1.72 1.72a.75.75 0 1 0 1.06-1.06L13.06 12l1.72-1.72a.75.75 0 1 0-1.06-1.06L12 10.94l-1.72-1.72Z',
      ],
      eventName: 'contactUs',
      buttonColor: 'warning',
    };

    if (!this.license)
      if (!this.activeLicense) {
        return [editLicense];
      }

    return [removeLicense, editLicense];
  }

  constructor(
    private _authService: AuthService,
    private _companyService: CompanyService,
    private _license: LicenseFormFactory,
    private _utilsService: UtilsService
  ) {}

  public handleButtonClick(event: string): void {
    switch (event) {
      case 'addLicense':
        this._addLicense();
        break;
      case 'contactUs':
        this._contactUs();
        break;
      default:
        break;
    }
  }

  private _addLicense(): void {
    const ref = this._license.create();

    ref.instance.submitted
      .pipe(
        switchMap((r) => {
          this.loading = true;
          return this._companyService.patchLicense(r);
        }),
        tap(() => {
          window.location.reload();
        }),
        catchError((err) => this._utilsService.errorHandler(err)),
        tap(() => {
          this.loading = false;
        })
      )
      .subscribe();
  }

  private _contactUs(): void {
    window.location.href = `mailto:supporto@notifyapp.it?subject=Disdetta&body=Vorrei richiedere la disdetta del mio account Notify con licenza ${this.license.publicKey}. Grazie.`;
  }
}
